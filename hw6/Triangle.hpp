...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    float invS1dotE1 = 1 / dotProduct(s1, edge1);
    // check if det = 0
    if (invS1dotE1 < EPSILON)
    {
        return intersection;
    }

    float S2dotE2 = dotProduct(s2, edge2);
    float S1dotS = dotProduct(s1, s);
    float S2dotD = dotProduct(s2, ray.direction);

    // Calculate tnear and barycentric coordinates of hit point.
    ray.t = invS1dotE1 * S2dotE2;
    u = invS1dotE1 * S1dotS;
    v = invS1dotE1 * S2dotD;
    ...
    if ( ray.t > EPSILON && (1 - u - v) > 0.f && u > 0.f && v > 0.f )
    {
        ...
        intersection.coords = ray.origin + ray.direction * ray.t;
        intersection.normal = normal;
        intersection.distance = ray.t;
